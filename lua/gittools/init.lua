...
---   GitTool logthis [<opt>...] [<rev>]        like log, scoped to the current
---                                             buffer's file
...
function M.run(_, args)
    ...
    elseif sub == "logthis" then
        local log_opts, revs = _split_opts({ unpack(args, 2) })
        if #revs > 1 then
            _notify("GitTool logthis takes at most one revision", vim.log.levels.ERROR)
            return
        end
        logtool.logthis({ rev = revs[1], args = log_opts })
...
function M.complete(_, rest, arg_lead)
    if #rest == 0 then
        return { "diff", "diffpaths", "diffthis", "log", "logthis", "graph",
            "stashlist", "blame", "merge" }
    end
    ...
    elseif sub == "diffthis" then
        return git.refs()
    elseif sub == "logthis" then
        -- No path to complete: the path is the current buffer's file.
        local out = {}
