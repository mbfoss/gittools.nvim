...
--- The hunk containing the cursor. Strictly containing: the accept maps edit
--- whatever they land on, so acting on a region the cursor is merely near would
--- rewrite text the user was not looking at.
---@param session GitTools.MergeSession
---@return GitTools.MergeHunk?
---@return integer? idx
local function _hunk_at_cursor(session)
    ...
    for i, h in ipairs(session.hunks) do
        if lnum >= h.s_lnum and lnum <= h.e_lnum then return h, i end
    end
    return nil, nil
end
...
local function _accept(session, which)
    ...
    if not h then
        _notify("Not inside a conflict block", vim.log.levels.WARN)
        return
    end
...
-- The conflict maps hang off these rather than off `x` directly. `$MERGED` is
-- the user's real file and gets hand-edited, so a bare `x` (delete character)
-- must not sit out `'timeoutlen'` waiting to see whether `xc` follows -- and
-- that ambiguity is settled from the keymap table alone, before any code of ours
-- runs, so the only way to avoid the wait is for no `x`-prefixed mapping to
-- exist. Hence the indirection: `x` is one unambiguous mapping whose <expr>
-- picks a <Plug> from the cursor position. Inside a conflict it yields the
-- prefix and the accept maps hang off that; anywhere else it yields a plain `x`
-- that fires at once, nothing longer being able to match. Inside a conflict a
-- lone `x` still deletes, just after the timeout -- there the user is reaching
-- for `xc` / `xi` anyway.
local _PLUG     = "<Plug>(GitToolsMergeX)"
local _PLUG_RAW = "<Plug>(GitToolsMergeXRaw)"
...
local function _set_keymaps(session)
    ...
    map(_PLUG .. "c", function() _accept(session, "ours") end, "Accept the current change (ours) for this conflict")
    map(_PLUG .. "i", function() _accept(session, "theirs") end, "Accept the incoming change (theirs) for this conflict")
    map(_PLUG .. "b", function() _accept(session, "both") end, "Accept both changes, current first")
    map(_PLUG .. "a", function() _accept(session, "base") end, "Accept the base (common ancestor)")
    map(_PLUG .. "d", function() _open_diff(session) end, "Toggle the $LOCAL | $MERGED | $REMOTE three-way diff")

    -- Both fall back to the builtin: `_PLUG` is a complete mapping as well as a
    -- prefix, so a lone `x` inside a conflict still deletes once the timeout
    -- lapses, and `_PLUG_RAW` is the whole of `x` outside one.
    map(_PLUG, "x", "Delete character")
    map(_PLUG_RAW, "x", "Delete character")

    map("x", function()
        return _hunk_at_cursor(session) and _PLUG or _PLUG_RAW
    end, "Resolve the conflict under the cursor, else delete character",
