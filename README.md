...
`diff`, `diffpaths`, `log`, `logthis`, `graph` and `stashlist` open in a tab
of their own.
...
`gittools://graph/v1.2.0:src`, `gittools://stashlist` -- so the
...
`GitTool log` scoped to the current buffer's file, without having to name it --
what `GitTool diffthis` is to `GitTool diff`:
...
The view, the keys and the `git log` options are `GitTool log`'s. The only
difference is where the path comes from: the buffer's file, so a revision is
the only positional it takes, and a `--` or a path of your own is not accepted.
...
down, since they rewrite whole blocks and a block you are not looking at is the
wrong thing to rewrite. Use `]x` / `[x` to get to one.

`$MERGED` is your real working-tree file and you will often hand-edit it, so `x`
stays out of the way when there is no conflict under the cursor: there it is the
plain builtin, delete-character, with no `'timeoutlen'` wait. Only inside a
conflict does it become a prefix -- and there a lone `x` still deletes, once the
timeout lapses. The corollary is that outside a conflict `xc` is read as vim
reads it, a delete followed by a pending change operator, rather than as a
missed resolve.
